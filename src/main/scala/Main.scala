...
//    var preProcessedTargetOntology: RDD[(String, String, String)] = p.ReConstructOntology(targetOntology)
//    println("############################## Preprocessed Target Ontology ##############################")
//    preProcessedTargetOntology.foreach(println(_))
//
//    var preProcessedSourceOntology: RDD[(String, String, String)] = p.ReConstructOntology(sourceOntology)
...
//
    ...
    var sourceClassesWithTranslations: RDD[(String, List[String])] = trans.Translate(preprocessedSourceClasses).distinct()
    println("All source with all translations "+sourceClassesWithTranslations.count())
    sourceClassesWithTranslations.foreach(println(_))
//    var soureClassesWithBestTranslation: RDD[(String, String)] = sourceClassesWithTranslations.map{case x => if (x._2.length>1) (x._1,trans.GetBestTranslation(x._2)) else (x._1,x._2.head)}
//var soureClassesWithBestTranslation: RDD[(String, String)] = sourceClassesWithTranslations.map{case x => if (x._2.length>1) (x._1,x._2.head) else (x._1,x._2.head)}
//var soureClassesWithBestTranslation: RDD[(String, List[String], List[String])] = sourceClassesWithTranslations.map(x => (x._1,x._2,trans.GetBestTranslation(x._2)))
    var soureClassesWithBestTranslation: RDD[(String, List[String], List[String])] = sourceClassesWithTranslations.map(x => (x._1,x._2,trans.GetBestTranslation(x._2))).filter(y=>y._3.length == 1)
...
//    relevantTranslations.foreach(println(_))
                ...
                //####################### Recreating the source ontologyTriples #####################################
            ...
            /*
                           ...
                           var translatedSourceOntology: RDD[(String, String, String)] = sor.ReconstructOntology(sourceOntology,sourceClassesWithTranslatedSynonyms)//.cache()
                       ...
                       //    println("Matching Two Ontologies")
                           var targetOntologyWithoutURI: RDD[(String, String, String)] = targetOntology.map{case(x)=> if (x.getObject.isLiteral)(x.getSubject.getLocalName,x.getPredicate.getLocalName,x.getObject.getLiteral.toString)else (x.getSubject.getLocalName,x.getPredicate.getLocalName,x.getObject.getLocalName)}
                               val m = new MatchingTwoOntologies()
                               var sTrans = m.Match(translatedSourceOntology,targetOntologyWithoutURI)
                           println("|       source triples (before using machine translation) needed for enrichment        |")
                           sTrans.foreach(println(_))
                           ...
                           //    var translatedTriples: RDD[(String, String, String)] = mt.Translate(outputStrings,translations)
                               var translatedTriples = mt.Translate(sTrans,translations)
